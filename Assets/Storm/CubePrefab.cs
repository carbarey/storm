...
public class CubePrefab : MonoBehaviour
{
    ...
    private Vector3 randomInteriorAttraction;
    ...
    void Start()
    {
        ...
        randomInteriorAttraction = RandomPointInTriangle(mySphereLeft.transform.position, mySphereRight.transform.position, mySphereHead.transform.position);
    }
    ...
    void Update()
    {
        ...
        if (moving)
        {
            ...
            gameObject.GetComponent<Rigidbody>().AddForce((triangleCenter - transform.position).normalized * strength * strength2);
        ...
        else
        {
            ...
            if (Random.value>0.99f) randomInteriorAttraction = RandomPointInTriangle(mySphereLeft.transform.position, mySphereRight.transform.position, mySphereHead.transform.position);
            gameObject.GetComponent<Rigidbody>().AddForce((randomInteriorAttraction - transform.position) * interiorStrength);
    ...
    Vector3 RandomPointInTriangle(Vector3 p1, Vector3 p2, Vector3 p3)
    {
        float r1 = Random.value;
        float r2 = Random.value;

        return ((1f - Mathf.Sqrt(r1)) * p1 + (Mathf.Sqrt(r1) * (1 - r2)) * p2 + (r2 * Mathf.Sqrt(r1)) * p3);
    }
}
